...
import { fileOptions, AxolotlAPI, ProcessError, ProcessSuccess } from '../types'
...
const api: AxolotlAPI = {
  ...
  fs: {
    ...
    processImages: async (
      ...
      try {
        const result = await ipcRenderer.invoke('process-images', paths, model)
        return result as ProcessSuccess | ProcessError
      } catch (error) {
        return {
